...
def create_question( question_id ):        
    ...
    if request.method=='POST' and form.validate_on_submit():
        ...
        db.session.commit()
        return redirect( "%s#comment_%s"%(url_for('question.detail', question_id=question_id), comment.id))
...
def modify_question( comment_id ):
    ...
    if request.method=='POST':
        ...
        if form.validate_on_submit():
            ...
            db.session.commit()
            return redirect( "%s#comment_%s"%(url_for('question.detail', question_id = comment.question.id),comment.id))
    else:
        form = CommentForm( obj=comment)
    return render_template('comment/comment_form.html', form=form)

              
@bp.route( '/delete/question/<int:comment_id>', methods=('GET','POST'))
@login_required
...
def create_answer( answer_id ):        
    ...
    if request.method=='POST' and form.validate_on_submit():
        ...
        return redirect( "%s#comment_%s"%(url_for('question.detail', question_id=answer.question_id),comment.id))
    return render_template( 'comment/comment_form.html', form=form)
...
def modify_answer( comment_id ):
    ...
    if request.method=='POST':
        ...
        if form.validate_on_submit():
            ...
            comment.modify_date = datetime.now()
            db.session.commit()
            return redirect( "%s#comment_%s"%(url_for('question.detail', question_id = comment.answer.question.id),comment.id))
    else:
        form = CommentForm( obj=comment)
    return render_template('comment/comment_form.html', form=form)

              
@bp.route( '/delete/answer/<int:comment_id>', methods=('GET','POST'))
@login_required
